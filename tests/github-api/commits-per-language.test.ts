...
describe('commit contributions on github', () => {
    it('should get correct commit contributions', async () => {
        ...
        const totalContributions = await getCommitLanguage('xshubhamg', [], 'token');
    ...
    it('should throw error when api failed', async () => {
        ...
        await expect(getCommitLanguage('xshubhamg', [], 'token')).rejects.toThrow('GitHub api failed');
    });

    it('should do a case-insensitive comparison for language exclusion', async () => {
        mock.onPost('https://api.github.com/graphql').reply(200, data);
        const repoData = await getCommitLanguage('xshubhamg', ['jupyter notebook'], 'token');
